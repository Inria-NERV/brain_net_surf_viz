function h = my_simpleBrainSurfaceHemisphere_cData_colorNoMotif_rangeHemisph(range  , centerPatch , alphaBrain , hemisphere , nodeColors )
%% adapted by Remy Ben Messaoud from 
...
%% put both hemispheres vertices together just to deal with range
load('mapVertex2parcel_left.mat')
load(['simple_brain_surface_hemisphere_' 'lh' '.mat']);

hemisphereVerticesIdx = hemisphereSurface.hemisphereVerticesIdx;
hemisphereFaces = hemisphereSurface.hemisphereFaces;
hemisphereVertices = brain.vertices(hemisphereVerticesIdx , :);

nVerticesLeft = size(hemisphereVertices , 1);
vertexColor = zeros(nVerticesLeft , 3);
for kVert = 1 : nVerticesLeft
    vertexColor(kVert ,:)= nodeColors(vert2parcelMap(kVert) , :);
end
allVertexColor = vertexColor;

tmp_shading_colorHem = brain.shading_pre(hemisphereVerticesIdx) * diff(range);
tmp_shading_colorHem = tmp_shading_colorHem - min(tmp_shading_colorHem) + range(1);
shading_colorHem = repmat(tmp_shading_colorHem,1,3);

shading_colorHemAll = shading_colorHem;
%%%%% right
load(['simple_brain_surface_hemisphere_' 'rh' '.mat']);
hemisphereVerticesIdx = hemisphereSurface.hemisphereVerticesIdx;
hemisphereFaces = hemisphereSurface.hemisphereFaces;
hemisphereVertices = brain.vertices(hemisphereVerticesIdx , :);
load('mapVertex2parcel_right.mat')
nVerticesRight = size(hemisphereVertices , 1);
vertexColor = zeros(nVerticesRight , 3);
for kVert = 1 : nVerticesRight
    vertexColor(kVert ,:)= nodeColors(vert2parcelMap(kVert) , :);
end

allVertexColor = [allVertexColor;vertexColor];
...
shading_colorHemAll = [shading_colorHemAll;shading_colorHem];
%%%%%%%%%
vertexColorFinalBoth = allVertexColor + shading_colorHemAll;
vertexColorFinalBoth = (vertexColorFinalBoth - min(min(vertexColorFinalBoth)))/...
    (max(max(vertexColorFinalBoth)) - min(min(vertexColorFinalBoth)));
...
hemisphereVertices = brain.vertices(hemisphereVerticesIdx , :);
    ...
    vertexColorFinal = vertexColorFinalBoth(1 : nVerticesLeft,:);
elseif strcmp(hemisphere , 'rh')
    load(['simple_brain_surface_hemisphere_' 'rh' '.mat']);
hemisphereVerticesIdx = hemisphereSurface.hemisphereVerticesIdx;
hemisphereFaces = hemisphereSurface.hemisphereFaces;
hemisphereVertices = brain.vertices(hemisphereVerticesIdx , :);
    vertexColorFinal = vertexColorFinalBoth(1 + nVerticesLeft :end,:);

end

%% deal with color
% if strcmp(hemisphere , 'lh')
%     load('/Users/remy.benmessaoud/ownCloud/remy.benmessaoud/MATLAB/UK-biobank/parcellations_code/mapVertex2parcel_left.mat')
% end
% if strcmp(hemisphere , 'rh')
%     load('/Users/remy.benmessaoud/ownCloud/remy.benmessaoud/MATLAB/UK-biobank/parcellations_code/mapVertex2parcel_right.mat')
% end
